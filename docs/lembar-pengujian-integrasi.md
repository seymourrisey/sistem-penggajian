...
- **Cakupan status code:** Seluruh 4 status code utama pada kontrak error response (`section 4.2` project-design.md) sudah tercakup berulang kali lintas 5 endpoint berbeda — 200 (No. 5, 8, 11, 12 DELETE call, 16-18, 19-20, 22), 201 (No. 1, 14, 15, dan seluruh setup entity di berbagai skenario), 400 (No. 2, 4, 7, 10), 404 (No. 6, 9, 13, 16, 23), 409 (No. 3, 15, 21). 500 tidak diuji secara eksplisit (sulit dipicu deterministik tanpa memutus koneksi database secara sengaja); jalur ini tercakup tidak langsung lewat `mapKaryawanError`/`mapPayrollError`/`mapDepartemenError`/`mapKomponenGajiError` default case di masing-masing handler.
